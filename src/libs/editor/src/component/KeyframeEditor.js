...
function KeyframeEditor() {
  ...
  function onClickAddKeyframe() {
    ...
    if (currentLayer.layerType === 'image') {
      currentLayer.addKeyframe(
        new Keyframe(
          masterManager.sceneTimer.getPlayTime(),
          new Vector2D(masterManager.frontCanvas.width / 2, masterManager.frontCanvas.height / 2),
          new Vector2D(1, 1),
          0,
          1,
        ),
  ...
  function renderKeyframes(context) {
    ...
    for (var i = 0; i < keyframes.length; i++) {
      ...
      if (
        masterManager.sceneManager.getCurrentScene().layerList[currentLayerIndex].layerType ===
        'text'
      ) {
        let attr_idx = 4;
      ...
      for (var i = 0; i < keyframes.length; i++) {
        ...
        if (keyframes[i].isMouseDragging) {
          ...
          if (mouseX <= horizontalLineStartOffsetX - 25) {
            ...
            let progressRate =
              mouseX / parseFloat(horizontalLineStartOffsetX + horizontalLineLength);
    ...
    <>
      ...
      {keyframeEditModalOpened ? (
        <KeyframeModal
          ...
          Keyframes={masterManager.sceneManager
            .getCurrentScene()
            .layerList[masterManager.sceneManager.currentLayerIndex].getKeyframes()}
          OnCancelClick={() => {
            setKeyframeEditModalOpen(false);
          ...
          <div>
            <button
              onClick={() => {
                ...
                let currentLayerIndex = masterManager.sceneManager.currentLayerIndex;
                let currentLayer =
                  masterManager.sceneManager.getCurrentScene().layerList[currentLayerIndex];
                let keyframes = currentLayer.getKeyframes();
